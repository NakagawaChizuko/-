  ...
  for (let rowIndex = 1; rowIndex < rows.length; rowIndex += 1) {
    ...
    for (const block of dayBlocks) {
      ...
      if (!assignmentMap.has(key)) {
        assignmentMap.set(key, { teamLeads: new Set(), recorders: new Set() });
      }
...
function buildRosterDayBlocks(headerRow) {
  ...
  for (let col = TEAM_ROSTER_DAY_COL_START; col <= TEAM_ROSTER_DAY_COL_END; col += 1) {
    const text = value(headerRow[col - 1]);
...
function applyTeamRosterAutofill() {
  ...
  const key = `${day}-${normalizeRosterTeam(team)}`;
  const assignment = teamRosterAssignmentMap.get(key);
  if (!assignment) {
    return;
  }
  const leads = Array.from(assignment.teamLeads).join(", ");
  const recorders = Array.from(assignment.recorders).join(", ");
